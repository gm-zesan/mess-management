...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold mb-6">Create Month</h1>
        ...
        <form action="{{ route('months.store') }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <select name="status" id="status" class="w-full px-4 py-2 border rounded-lg @error('status') border-red-500 @enderror" required>
                    <option value="{{ MonthStatusEnum::ACTIVE->value }}" {{ old('status') === MonthStatusEnum::ACTIVE->value ? 'selected' : '' }}>
                        {{ MonthStatusEnum::ACTIVE->label() }}
                    </option>
                    <option value="{{ MonthStatusEnum::CLOSED->value }}" {{ old('status') === MonthStatusEnum::CLOSED->value ? 'selected' : '' }}>
...
@else
    <div class="container mx-auto px-4 py-8">
        <div class="alert alert-danger" role="alert">
            <strong>Access Denied!</strong> You don't have permission to create months.
        </div>
